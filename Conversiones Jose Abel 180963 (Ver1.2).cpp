...
int UserInputService (char UserChoice[], float Delay){
	if (strcmp(UserChoice, "c") == 0) {  // Compare strings using strcmp
		...
		printf("\nEl resultado de su conversion es el siguiente: %.3f, tome nota del resultado.\n",ConversionC);
		...
		printf("\nEl resultado de su conversion es el siguiente: %.3f grados Farenheit, tome nota del resultado.\n",ConversionF);
	
	} else if (strcmp (UserChoice, "f") != 0 || strcmp,(UserChoice, "c") != 0){
		char Error[] = " \n Error! El input concretado no cumple con ninguna condicional. \n";
		int Leng2 = sizeof(Error) - 1;
		StringPrint (Error, Leng2, Delay);
	}
...
int main() {
    ...
    while (1) {
        ...
        int Leng1 = sizeof(TypeInput) - 1;  // Subtraer 1 para excluir el terminador null ('\0')
